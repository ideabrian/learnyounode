...
// get the path from the file input
var input = process.argv[2];
// get the extension to filter on 
var ext = process.argv[3];

// call the imported function with input, ext and a callback function
filter(input, ext, function (err, files) {
	// if there's an error, notify somebody - callbacks should handle errors
	if (err)
		return console.error('There was an error:', err)
	// our function's callback returns an array of file-names that match our criteria

	files.forEach(function (file) {
		// loop over the array and console.log()
	  console.log(file);
	});
});
